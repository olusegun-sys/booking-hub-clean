...
const FROM_EMAIL = isValidFromEmail(RAW_FROM_EMAIL)
  ? RAW_FROM_EMAIL
  : "onboarding@resend.dev";
...
console.log(
  ...
  RESEND_API_KEY
    ? "set (" + RESEND_API_KEY.substring(0, 8) + "...)"
    : "❌ MISSING",
);
...
console.log(
  "[Email] - FROM_EMAIL (valid):",
  isValidFromEmail(RAW_FROM_EMAIL) ? "yes" : "no",
);
console.log(
  "[Email] - From header will be:",
...
if (!isValidFromEmail(RAW_FROM_EMAIL)) {
  console.error(
    "[Email] ⚠️  FROM_EMAIL is not a valid email — falling back to onboarding@resend.dev",
  );
  console.error(
    "[Email] ⚠️  Fix this in Render → booking-backend-clean → Environment → FROM_EMAIL",
  );
}
...
function buildBookingEmail(booking, business, labels) {
  ...
  var subtitle = (labels.headerSubtitle || "").replace(
    "{business}",
    businessName,
  );
  ...
  var primaryDateRow = primaryDateValue
    ? '<div class="detail-row"><span class="detail-label">' +
      ...
      formatDate(primaryDateValue) +
      "</span></div>"
    : "";

  // WHY: Secondary date row only appears for stays (check-out)
  var secondaryDateValue = labels.secondaryDateValueField
    ? booking[labels.secondaryDateValueField]
    : null;
  var secondaryDateRow =
    labels.secondaryDateLabel && secondaryDateValue
      ? '<div class="detail-row"><span class="detail-label">' +
        labels.secondaryDateLabel +
        '</span><span class="detail-value">' +
        formatDate(secondaryDateValue) +
        "</span></div>"
      : "";
  ...
  var guestsRow =
    labels.guestsLabel && guestsValue
      ? '<div class="detail-row"><span class="detail-label">' +
        labels.guestsLabel +
        '</span><span class="detail-value">' +
        guestsValue +
  ...
  return (
    ...
    businessName +
    ". Your " +
    (labels.subjectPrefix.toLowerCase().includes("order")
      ? "order"
      : labels.subjectPrefix.toLowerCase().includes("appointment")
        ? "appointment"
        : "booking") +
    " has been confirmed.</p>" +
    '<div class="details-card">' +
    '<div style="margin-bottom:20px;display:flex;justify-content:space-between;align-items:center"><h3 style="margin:0;color:#0f172a">Details</h3><span class="badge">' +
    reference +
    "</span></div>" +
    primaryDateRow +
    secondaryDateRow +
    guestsRow +
    '<div class="detail-row"><span class="detail-label">Total Amount</span><span class="detail-value" style="color:#10b981;font-size:20px">' +
    formatNaira(totalAmount) +
    "</span></div>" +
    "</div>" +
    '<div class="business-info"><h3 style="margin-top:0;color:#0f172a">' +
    businessName +
    "</h3>" +
    (businessLocation
      ? '<p style="color:#64748b;margin:4px 0">' + businessLocation + "</p>"
      : "") +
    (business.phone
      ? '<p style="color:#64748b;margin:4px 0">' + business.phone + "</p>"
      : "") +
    "</div>" +
...
var welcomeTemplate = function (business) {
  return (
    '<!DOCTYPE html><html><head><style>body{font-family:"Inter",Arial,sans-serif;line-height:1.6;color:#1e293b;margin:0;padding:0}.container{max-width:600px;margin:0 auto;background:white}.header{background:linear-gradient(135deg,#4f46e5 0%,#7c3aed 100%);color:white;padding:40px 30px;text-align:center}.header h1{margin:0;font-size:26px;font-weight:700}.content{padding:30px;background:#f8fafc}.card{background:white;border-radius:12px;padding:24px;margin:24px 0;box-shadow:0 2px 8px rgba(0,0,0,0.04)}.footer{text-align:center;padding:30px;color:#64748b;font-size:13px;border-top:1px solid #e2e8f0}.badge{background:#fef3c7;color:#92400e;padding:4px 12px;border-radius:30px;font-size:12px;font-weight:600;display:inline-block}.button{display:inline-block;background:#4f46e5;color:white !important;padding:14px 32px;border-radius:10px;text-decoration:none;font-weight:600;margin:20px 0}</style></head><body><div class="container"><div class="header"><h1>Welcome to Plazzaa!</h1><p style="margin:10px 0 0;opacity:0.9">Your booking page is being set up</p></div><div class="content"><p>Dear ' +
    business.name +
...
async function sendBookingConfirmation(booking, business) {
  ...
  console.log(
    "[Email] - Business type:",
    business ? business.business_type : "MISSING",
  );
  ...
  if (
    ...
    if (business.email !== booking.customer_email) {
      ...
      console.log(
        "[Email] Customer is also business owner — skipping duplicate",
      );
    }
  }
...
async function sendReminderEmail(booking, business) {
  return sendEmail({
    ...
    html:
      '<div style="font-family:Inter,Arial;max-width:600px;margin:0 auto"><div style="background:#4f46e5;color:white;padding:30px;text-align:center"><h1>Reminder: Your Booking Tomorrow!</h1></div><div style="padding:30px;background:#f8fafc"><p>Dear ' +
      booking.customer_name +
      ",</p><p>This is a friendly reminder about your booking tomorrow at " +
      business.name +
      '.</p><p style="background:#eef2ff;padding:12px;border-radius:8px;font-family:monospace">' +
      booking.booking_reference +
      '</p></div><div style="text-align:center;padding:30px;color:#64748b;font-size:14px"><p>Plazzaa — Built for Nigerian businesses</p></div></div>',
  });
}
...
function formatSlot(startIso, endIso) {
  ...
  var startTime = start.toLocaleTimeString("en-NG", {
    timeZone: "UTC",
    hour: "2-digit",
    minute: "2-digit",
  });
...
function validatedTemplate(booking, business, service) {
  ...
  return (
    ...
    businessName +
    " has confirmed your payment</p></div>" +
    '<div style="padding:30px;background:#f8fafc">' +
    '<p style="font-size:17px;margin-top:0">Hi ' +
    (booking.customer_name || "there") +
    ",</p>" +
    ...
    (business && business.address
      ? '<p style="color:#64748b;margin:0">' + business.address + "</p>"
      : "") +
    (contact
      ? '<p style="color:#64748b;margin:6px 0 0">' + contact + "</p>"
      : "") +
    "</div>" +
    '<p style="color:#64748b;font-size:14px">Need to change something? Contact ' +
    businessName +
    " directly.</p>" +
    "</div>" +
...
function awaitingValidationTemplate(booking, business) {
  ...
  return (
    ...
    detailRow(
      "WhatsApp",
      booking.customer_whatsapp || booking.customer_phone || "Not provided",
    ) +
    detailRow("Date", slot.date) +
    detailRow("Time", slot.time || "To be confirmed") +
    detailRow("Amount", formatNaira(booking.total_amount)) +
...
async function sendAwaitingValidation(booking, business) {
  if (
    !business ||
    !business.email ||
    String(business.email).indexOf("@") === -1
  ) {
    console.log(
      "[Email] No business email on file - skipping validation notice",
    );
