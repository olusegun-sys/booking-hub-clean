...
// UPDATED (22 Sept 2026): /api/businesses now returns rooms[] and filters is_active=true
//                        WHY: homepage needs the business's own uploaded image (logo or
//                        primary item image), and only approved+active businesses should show.
...
if (!supabaseUrl || !supabaseKey) {
  ...
  console.error(
    "[server.js] SUPABASE_KEY:",
    supabaseKey ? "Present" : "Missing",
  );
  console.error(
    "[server.js] Please check your .env file in the server/ folder",
  );
  process.exit(1);
}
...
function logEmailResult(label, result) {
  ...
  if (result.success) {
    console.log(
      "[Email] ✅ " + label + ": delivered — ID:",
      result.data && result.data.messageId ? result.data.messageId : "n/a",
    );
  } else {
    console.error(
      "[Email] ❌ " + label + ": FAILED —",
...
// >>> CHANGED 22 Sept 2026: /api/businesses now joins rooms and requires is_active=true
// WHY: The homepage needs each business's own uploaded image (logo_url or the
// primary image on their first item). Nested select brings rooms along in one
// request instead of N+1 fetches. is_active=true guards against any approved
// but deactivated business leaking onto the public homepage.
app.get("/api/businesses", async function (req, res) {
  try {
    const { data, error } = await supabase
      .from("businesses")
      .select("*, rooms(id, images, name)") // WHY: include rooms so homepage can use primary item image
      .eq("status", "approved") // WHY: only vetted businesses
      .eq("is_active", true) // WHY: also require active flag
...
app.get("/api/businesses/search/category", async function (req, res) {
  try {
    ...
    res
      ...
      .json({
        error: "Search failed",
        details: "An error occurred during search",
      });
  }
});
...
app.get("/api/domain-info", async function (req, res) {
  try {
    ...
    if (req.detectedBusiness) {
      return res.json({
        success: true,
        business: req.detectedBusiness,
        source: req.domainSource || "custom-domain",
      });
    }
    res.json({
      success: false,
      message: "No business associated with this domain",
    });
  } catch {
...
app.post("/api/businesses/register", async function (req, res) {
  try {
    ...
    if (data) {
      sendWelcomeEmail(data)
        .then(function (result) {
          logEmailResult("Welcome (" + data.email + ")", result);
        })
        .catch(function (err) {
          console.error("[Email] ❌ Welcome threw an exception:", err);
        });
    }
    ...
    var safeNewBusiness = removeSensitiveFields(
      data,
      "password_hash",
      "password",
    );
...
app.post("/api/businesses/login", async function (req, res) {
  try {
    ...
    var safeBusiness = removeSensitiveFields(
      business,
      "password_hash",
      "password",
    );
    ...
    try {
      ...
      if (req.body.status === "approved" && data) {
        sendApprovalEmail(data)
          .then(function (result) {
            logEmailResult("Approval (" + data.email + ")", result);
          })
          .catch(function (err) {
            console.error("[Email] ❌ Approval threw an exception:", err);
          });
      }
...
app.delete(
  ...
  async function (req, res) {
    try {
      ...
      res.json({
        success: true,
        message: business.name + " has been permanently deleted.",
      });
    } catch (error) {
...
app.get("/api/admin/stats", authenticateAdmin, async function (req, res) {
  try {
    ...
    var totalRevenue = revenueData
      ? revenueData.reduce(function (sum, b) {
...
app.get("/api/businesses/:businessId/can-book", async function (req, res) {
  try {
    ...
    const remaining = Math.max(
      0,
      business.booking_limit - business.current_booking_count,
    );
    const usage = Math.min(
      100,
      (business.current_booking_count / business.booking_limit) * 100,
    );
    ...
    res.json({
      ...
      data: {
        ...
        isPremium:
          business.subscription_status === "premium" ||
          business.subscription_status === "pro",
        message: canBook
          ? "You can accept bookings"
          : "Booking limit reached. Please upgrade to continue.",
      },
    });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to check booking capacity" });
  }
});
  ...
  async function (req, res) {
    try {
      ...
      const { data: business, error } = await supabase
        .from("businesses")
        .select(
          "booking_limit, current_booking_count, subscription_status, subscribed_at",
        )
        .eq("id", businessId)
        .single();
...
app.post(
  "/api/businesses/:businessId/upgrade-request",
  authenticateBusiness,
  async function (req, res) {
    try {
      const { businessId } = req.params;
      ...
      const ref =
        paymentReference ||
        `UPG-${Date.now()}-${Math.random().toString(36).substring(7).toUpperCase()}`;
      ...
      res.json({
        ...
        message:
          "Upgrade request created. Transfer the amount and we'll verify within 24 hours.",
      });
    } catch (error) {
      console.error("Upgrade request error:", error);
      res
        .status(500)
        .json({ success: false, error: "Failed to create upgrade request" });
    }
  },
);

app.post(
  "/api/admin/subscription/verify",
  authenticateAdmin,
  async function (req, res) {
    try {
      ...
      if (!upgradeId) {
        return res
          .status(400)
          .json({ success: false, error: "Upgrade ID is required" });
      }
      ...
      if (fetchError || !upgrade) {
        return res
          .status(404)
          .json({ success: false, error: "Upgrade request not found" });
      }

      if (upgrade.status !== "pending") {
        return res
          .status(400)
          .json({ success: false, error: "Upgrade already processed" });
      }
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to verify upgrade" });
    }
  },
);

app.get(
  "/api/admin/pending-upgrades",
  authenticateAdmin,
  async function (req, res) {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to fetch pending upgrades" });
    }
  },
);
  ...
  async function (req, res) {
    ...
    try {
      ...
      var { data, error } = await supabase
        .from("businesses")
        .select(
          "id, name, email, phone, city, state, logo_url, cover_image, business_type, slug, description, about_text, website, status, booking_limit, current_booking_count, subscription_status, subscribed_at",
        )
        .eq("id", businessId)
        .single();
...
app.put("/api/businesses/:id", authenticateBusiness, async function (req, res) {
  try {
    var { cover_image, logo_url, about_text, description, website, name } =
      req.body;
    var updateData = {};
...
app.post(
  "/api/businesses/:businessId/rooms/create",
  authenticateBusiness,
  async function (req, res) {
    try {
      var {
      ...
      if (!name || name.trim().length < 2) {
        return res
          .status(400)
          .json({ success: false, error: "Room name is required." });
      }
      if (!price_per_night || isNaN(price_per_night) || price_per_night <= 0) {
        return res
          .status(400)
          .json({ success: false, error: "A valid price is required." });
      }
...
app.post(
  "/api/businesses/:businessId/rooms/:roomId/upload-image",
  authenticateBusiness,
  async function (req, res) {
    try {
      ...
      if (fileBuffer.length > 5 * 1024 * 1024) {
        return res
          .status(400)
          .json({ success: false, error: "File size must be under 5MB." });
      }
      ...
      if (roomError || !room) {
        return res
          .status(404)
          .json({ success: false, error: "Venue not found or access denied." });
      }
      ...
      if (updateError) {
        ...
        return res.status(500).json({
          success: false,
          error:
            "Failed to update venue with new image: " + updateError.message,
        });
      }

      console.log(
        "[Venue Upload] Successfully uploaded image for room:",
        roomId,
      );
...
app.delete(
  "/api/businesses/:businessId/rooms/:roomId/images",
  authenticateBusiness,
  async function (req, res) {
    try {
      const { businessId, roomId } = req.params;
      ...
      if (!currentImages.includes(imageUrl)) {
        return res
          .status(404)
          .json({ success: false, error: "Image not found in this venue." });
      }
      ...
      try {
        ...
        if (parts.length === 2) {
          ...
          if (deleteError) {
            console.log(
              "[Venue Delete] Storage delete warning:",
              deleteError.message,
            );
          } else {
...
// OPERATING HOURS
app.get(
  "/api/businesses/:businessId/operating-hours",
  authenticateBusiness,
  async function (req, res) {
    try {
      var { data, error } = await supabase
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to fetch operating hours" });
    }
  },
);
    ...
    try {
      ...
      if (req.body.operatingHours && req.body.operatingHours.length) {
        var { data, error } = await supabase
          .from("operating_hours")
          .insert(
            req.body.operatingHours.map(function (h) {
              return { ...h, business_id: req.params.businessId };
            }),
          )
          .select();
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to update operating hours" });
    }
  },
);
...
// BLOCKED DATES
app.get(
  "/api/businesses/:businessId/blocked-dates",
  authenticateBusiness,
  async function (req, res) {
    try {
      var { data, error } = await supabase
...
app.post(
  "/api/businesses/:businessId/block-date",
  authenticateBusiness,
  async function (req, res) {
    try {
      var { data, error } = await supabase
...
app.delete(
  "/api/businesses/:businessId/block-date/:date",
  authenticateBusiness,
  async function (req, res) {
    try {
      await supabase
...
app.get(
  "/api/businesses/:businessId/phone-numbers/public",
  getPublicPhoneNumbersRoute,
);

app.put(
  "/api/businesses/:businessId/features",
  authenticateBusiness,
  updateFeaturesRoute,
);
app.put(
  "/api/businesses/:businessId/amenities",
  authenticateBusiness,
  updateAmenitiesRoute,
);
app.put(
  "/api/businesses/:businessId/area-guide",
  authenticateBusiness,
  updateAreaGuideRoute,
);
app.put(
  "/api/businesses/:businessId/phone-numbers",
  authenticateBusiness,
  updatePhoneNumbersRoute,
);
app.put(
  "/api/businesses/:businessId/property-details",
  authenticateBusiness,
  updatePropertyDetailsRoute,
);

app.post(
  "/api/businesses/:businessId/gallery",
  authenticateBusiness,
  addGalleryImageRoute,
);
app.delete(
...
app.get(
  "/api/businesses/:businessId/booking-capacity",
  async function (req, res) {
    try {
      const { businessId } = req.params;
      ...
      const { data: business, error } = await supabase
        .from("businesses")
        .select(
          "current_booking_count, booking_limit, subscription_status, name",
        )
        .eq("id", businessId)
        .single();
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to check capacity" });
    }
  },
);
...
app.post("/api/bookings", async function (req, res) {
  try {
    ...
    if (!businessId) {
      return res
        .status(400)
        .json({ success: false, error: "Business ID is required" });
    }
    if (!customerEmail) {
      return res
        .status(400)
    ...
    if (currentCount >= limit) {
      ...
      return res.status(403).json({
        success: false,
        error:
          "Booking limit reached. Please upgrade your plan to continue accepting bookings.",
        limitReached: true,
    ...
    var emailDetails = {
      ...
      bookingDetails: bookingDetails || {
        ...
        paymentMethod:
          paymentMethod === "pay_at_venue" ? "Pay at Venue" : "Paystack",
      },
    };
    ...
    sendBookingConfirmation(emailDetails, business)
      .then(function (emailResults) {
        ...
        logEmailResult(
          "Customer booking confirmation (" + booking.customer_email + ")",
          emailResults.customer,
        );
        if (emailResults.owner) {
          logEmailResult(
            "Owner booking notification (" + business.email + ")",
            emailResults.owner,
          );
        }
      })
...
app.post("/api/create-payment", async function (req, res) {
  try {
    ...
    if (!bookingReference)
      return res
        .status(400)
        .json({ success: false, error: "Booking reference is required." });
    if (!email || !email.includes("@"))
      return res
        .status(400)
    ...
    if (booking.payment_status === "paid") {
      return res
        ...
        .json({
          success: false,
          error: "Booking already paid. Please contact support.",
        });
    }

    var result = await initializePayment({
      email: email,
      amount: amount,
...
app.post("/api/verify-payment", async function (req, res) {
  try {
    ...
    if (!reference)
      return res
        .status(400)
        .json({ success: false, error: "Payment reference is required." });
...
app.post("/api/upload-gallery-image", async function (req, res) {
  try {
    ...
    if (!businessId || !fileName || !fileData) {
      return res
        .status(400)
        .json({ error: "Business ID, file name, and file data are required" });
    }
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ error: "Failed to upload image to storage: " + error.message });
    }
  ...
  async function (req, res) {
    try {
      ...
      if ((count || 0) >= 5) {
        return res
          .status(400)
          .json({ error: "Gallery is full. Maximum 5 images allowed." });
      }
      ...
      var nextSortOrder =
        lastImage && lastImage.length > 0 ? lastImage[0].sort_order + 1 : 0;
      ...
      if (error)
        return res
          .status(500)
          .json({ error: "Failed to save gallery image: " + error.message });
...
app.put(
  "/api/businesses/:businessId/gallery/reorder",
  authenticateBusiness,
  async function (req, res) {
    try {
      var { businessId } = req.params;
...
app.post(
  "/api/businesses/generate-verification",
  authenticateBusiness,
  async function (req, res) {
    try {
      var businessId = req.businessId;
      ...
      if (error) {
        ...
        if (
          ...
          return res.status(500).json({
            success: false,
            error:
              "Database schema missing required columns. Please add: domain_verification_code, domain_verification_expires, domain_verified_at",
            details: error.message,
          });
        }
      ...
      res.json({
        ...
        warning:
          "This code expires in 24 hours. Please verify your domain before then.",
      });
    } catch (error) {
...
app.post(
  "/api/businesses/check-verification",
  authenticateBusiness,
  async function (req, res) {
    try {
      var businessId = req.businessId;
      ...
      if (!custom_domain) {
        return res
          .status(400)
          .json({ success: false, error: "Custom domain is required" });
      }
      ...
      if (!isValidDomainFormat) {
        return res.status(400).json({
          success: false,
          error:
            "Invalid domain format. Please enter a valid domain (e.g., book.yourbusiness.com)",
        });
      }
      ...
      if (business.domain_verification_expires) {
        ...
        if (new Date() > expiryDate) {
          return res.status(400).json({
            success: false,
            error:
              "Verification code has expired. Please generate a new code and try again.",
          });
        }
      }

      var result = await verifyDomainTxtRecord(
        custom_domain,
        business.domain_verification_code,
      );
      ...
      if (!result.verified) {
        return res.status(400).json({
          ...
          error:
            result.error ||
            "TXT record not found. Please add the verification code to your DNS and wait 10-30 minutes for propagation.",
          details: result,
        });
      }
      ...
      res.json({
        success: true,
        message:
          "Domain verified successfully! Your booking page will now be available at this domain.",
...
async function ensureSubscriptionTable() {
  try {
    ...
    if (checkError && checkError.message.includes("does not exist")) {
      console.log(
        "Please create subscription_upgrades table manually in Supabase SQL Editor.",
      );
    } else {
